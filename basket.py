...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        varkk = np.diag(cov_m)[:,None]
        prices = forward[:,None] * np.exp(-0.5*texp*varkk + np.sqrt(texp) * chol_m @ znorm_m)
...
def basket_price_norm_analytic(
    ...
    sigma = np.fmax(sigma , 1.0e-8)
    
    # 3. plug in the forward and volatility to the normal price formula
    d = (forward - strike) / (sigma* np.sqrt(texp)) 
    price = disc_fac * (cp * (forward - strike) * ss.norm.cdf(cp * d) + sigma* np.sqrt(texp) * ss.norm.pdf(d))
    # OR:
    # norm = pf.Norm(sigma, intr=intr, divr=divr)
    # price = norm.price(strike, forward@ weights[:,None], texp, cp=cp)
    
    return (price@ weights[:,None]).item() # price
